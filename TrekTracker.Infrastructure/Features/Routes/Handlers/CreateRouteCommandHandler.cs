...
public class CreateRouteCommandHandler : IRequestHandler<CreateRouteCommand, RouteResponseDto>
{
    private readonly TrekTrackerDbContext _context;

    public CreateRouteCommandHandler(TrekTrackerDbContext context)
    {
        _context = context;
    }

    public async Task<RouteResponseDto> Handle(CreateRouteCommand request, CancellationToken cancellationToken)
    {
        var route = new Route
        {
            ...
            StartLatitude = request.Request.StartLatitude,
            StartLongitude = request.Request.StartLongitude,
            EndLatitude = request.Request.EndLatitude,
            EndLongitude = request.Request.EndLongitude,
            Difficulty = request.Request.Difficulty,
            DistanceKm = request.Request.DistanceKm,
            ElevationGainM = request.Request.ElevationGainM,
            EstimatedHours = request.Request.EstimatedHours,
            Season = request.Request.Season,
